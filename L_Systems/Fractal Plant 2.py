    ...
    def logic(self):
        ...
        if self.string and self.char_counter < len(self.string) - 1:
            if self.string[self.char_counter] == 'F':
                self.start = self.end
                self.end = self.start + self.distance + np.random.uniform(-self.distance * 0.6, self.distance * 0.6)
            if self.string[self.char_counter] == '+':
                angle = np.pi / 12 + np.random.uniform(-np.pi / 12 * 0.8, np.pi / 12 * 0.8)
...
if __name__ == '__main__':
    ...
    input_string = 'F'
    simulation = Simulation(8, np.array([640, 1200]))
    input_string = simulation.iterate_string(input_string, 5)
