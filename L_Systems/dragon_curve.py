...
class Simulation:
    ...
    def logic(self):
        ...
        if self.string and self.char_counter < len(self.string) - 1:
            if self.string[self.char_counter] == 'F' or self.string[self.char_counter] == 'G':
                self.start = self.end
                self.end = self.start + self.distance
...
if __name__ == '__main__':
    ...
    input_string = 'F'
    simulation = Simulation(3, np.array([320, 320]))
    input_string = simulation.iterate_string(input_string, 12)
